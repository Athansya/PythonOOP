...
class InputNumber(pygwidgets.InputText):
    ...
    def __init__(self, window, loc, value='', fontName=None, fontSize=24, width=200, textColor=BLACK,
            backgroundColor=WHITE, focusColor=BLACK, initialFocus=False, nickName=None, callback=None,
        ...
        super().__init__(window, loc, value, fontName, fontSize, width, textColor, backgroundColor,
                focusColor, initialFocus, nickName, callback, mask, keepFocusOnSubmit)

    # Override handleEvent so we can filter for proper keys
    def handleEvent(self, event):
        if (event.type == pygame.KEYDOWN):
            # If it's not an editin or numeric key, ignore it
            ...
            if event.unicode == '.':
                if not self.allowFloatingNumber:
                    # If no floats, don't pass the period throught
                    return False
                if '.'in self.text:
