...
Objetivo: Utilizar la clase DisplayMoney.
'''
...
title = pygwidgets.DisplayText(
            window, (0, 40), 'Demo of InputNumber and DisplayMoney fields',
            fontSize=36, width=WINDOW_WIDTH, justified='center'
        )
inputCaption = pygwidgets.DisplayText(
                   window, (20, 150), 'Input money amount:',
...
outputCaption1 = pygwidgets.DisplayText(
                     window, (20, 300), 'Output dollars & cents: ',
                     fontSize=24, width=190, justified='right'
                 )
moneyField1 = DisplayMoney(
                  window, (230, 300), '', textColor=BLACK,
                  backgroundColor=WHITE, width=150
              )
outputCaption2 = pygwidgets.DisplayText(
                     window, (20, 400), 'Output dollars only: ',
                     fontSize=24, width=190, justified='right'
                 )
moneyField2 = DisplayMoney(
                  window, (230, 400), '', textColor=BLACK,
...
while True:
    ...
    for event in pygame.event.get():
        # if the vent was a click on the close box, quit pygame
        # and the program
        if event.type == pygame.QUIT:
            pygame.quit()
            sys.exit()

        # Pressing Return/Enter or clicking OK trigger action
